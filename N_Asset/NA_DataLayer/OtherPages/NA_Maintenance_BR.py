...
class NA_BR_Maintenance(models.Manager):
    def PopulateQuery(self,orderFields,sortIndice,pageSize,PageIndex,userName,columnKey,ValueKey,criteria=CriteriaSearch.Like,typeofData=DataType.VarChar):
        ...
        cur.execute("DROP TEMPORARY TABLE IF EXISTS T_Maintenance_Manager_" + userName )
        rs = ResolveCriteria(criteria, typeofData, columnKey, ValueKey)
        ...
        if orderFields != '':
            Query = "SELECT * FROM T_Maintenance_Manager_" + userName  + " ORDER BY " + orderFields + (" DESC" if sortIndice == "" else ' ' + sortIndice) + " LIMIT " + strLimit + "," + str(pageSize)
        else:
            Query = "SELECT * T_Maintenance_Manager_" + userName + " ORDER BY IDApp LIMIT " + strLimit + "," + str(pageSize)
        cur.execute(Query)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if statusForm == StatusForm.Input:
                    ...
                    Params = {'FK_Goods': data['fk_goods'], 'TypeApp': data['typeApp'], 'SerialNumber': data['serialNum'], 'FK_Maintenance': FKApp, 'CreatedBy': data['createdby']}
    ...
    def search_M_ByForm(self,searchText,orderFields,sortIndice,pageSize,PageIndex,userName,includeNotYetUsed=False):
        ...
        if int(PageIndex) <= 1:
            ...
            strLimit = str((int(PageIndex)-1) * int(pageSize))
        if orderFields == '':
            Query = """SELECT * FROM T_Search_Maintenance_""" + userName + """ ORDER BY goods """ + (" DESC" if sortIndice == "" else ' ' + sortIndice) #+ " LIMIT " + strLimit + "," + str(pageSize)
            ...
            Query = """SELECT * FROM T_Search_Maintenance_""" + userName + """ ORDER BY """ + orderFields + (" DESC" if sortIndice == "" else ' ' + sortIndice) #+ " LIMIT " + strLimit + "," + str(pageSize)
    ...
    def dataExist(self, **kwargs):
        ...
        filterdate = {'startdate__range': [parse(startdate1), parse(startdate2)], 'enddate__range': [parse(enddate1), parse(enddate2)]}
        xdata = super(NA_BR_Maintenance, self).get_queryset().filter(Q(fk_goods=fk_goods) & Q(serialnumber=SN)).filter(**filterdate)
        return xdata.exists()
    ...
    def getPersonalName(self,personname):
        data = super(NA_BR_Maintenance,self).get_queryset().values('personalname').filter(personalname=personname).distinct()
        if data:
            return data[0]
        else:
            return data
    def getMaintenanceBy(self,maintenance_by):
        data = super(NA_BR_Maintenance,self).get_queryset().values('maintenanceby').filter(maintenanceby=maintenance_by).distinct()
        if data:
            return data[0]
        else:
            return data
