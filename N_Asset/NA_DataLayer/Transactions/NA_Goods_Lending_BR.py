...
class NA_BR_Goods_Lending(models.Manager):
	def PopulateQuery(self,orderFields,sortIndice,pageSize,PageIndex,userName,columnKey,ValueKey,criteria=CriteriaSearch.Like,typeofData=DataType.VarChar):
		...
		cur.execute(Query)
		Query = """ CREATE TEMPORARY TABLE T_Lending_Manager_""" + userName  + """ ENGINE=MyISAM AS (SELECT ngl.idapp,L.territory,g.goodsname AS goods,ngd.TypeApp AS goodstype,ngd.serialnumber,L.lentby,S.sentby,ngl.DateLending AS lentdate,ngl.DateReturn as datereturn,ngl.interests,R.responsibleby,
					...
					LEFT OUTER JOIN(SELECT IDApp,Employee_Name AS lentby,territory FROM employee)L
