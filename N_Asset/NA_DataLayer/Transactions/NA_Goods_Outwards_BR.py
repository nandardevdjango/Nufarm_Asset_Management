...
class NA_BR_Goods_Outwards(models.Manager):
    def PopulateQuery(self, orderFields, sortIndice, pageSize, PageIndex, userName, columnKey, ValueKey, criteria=CriteriaSearch.Like, typeofData=DataType.VarChar):
        ...
        if orderFields != '':
            ...
            Query = """SELECT * FROM T_Outwards_Manager_""" + userName + \
    ...
    # Query get last trans in history
        ...
        if orderFields == '':
            Query = "SELECT * FROM Temp_F_Outwards_" + userName + " ORDER BY brandname " + \
                (" DESC" if sortIndice == "" else ' ' + sortIndice) + \
                " LIMIT " + strLimit + "," + str(pageSize)
        else:
            Query = "SELECT * FROM Temp_F_Outwards_" + userName + " ORDER BY " + orderFields + \
                (" DESC" if sortIndice == "" else ' ' + sortIndice) + \
                " LIMIT " + strLimit + "," + str(pageSize)
        cur.execute(Query)
        result = query.dictfetchall(cur)
    ...
    def getLastTrans(self, SerialNO):
        ...
        if int(row[0]) > 0:
            ...
            if int(fklending) > 0:
                ...
                if cur.rowcount > 0:
                    ...
                    if isFinished and isSucced:
                        lastInfo = 'Last maintenance by ' + \
                            str(row[0]) + ', date returned ' + \
                            endDate + ', ' + ' (goods is able to use)'
                    elif isFinished == True and isSucced == False:
                        lastInfo = 'Last maintenance by ' + \
                            str(row[0]) + ', date returned ' + endDate + \
                            ', ' + ' (goods is unable to use )'
                    elif not isFinished:
                        lastInfo = 'Last maintenance by ' + \
                            str(row[0]) + ', start date maintenance ' + \
                            starDate + ', ' + \
                            ' (goods is still in maintenance)'
                    ...
                    if lost_status == "F":
                        if int(fk_lost_lending) > 0:
                            ...
                            if cur.rowcount > 0:
                                ...
                                if row[1] is not None:
                                    starDate = parse(
                                        str(row[1])).strftime('%d %B %Y')
                                if row[2] is not None:
                                    endDate = parse(
                                        str(row[2])).strftime('%d %B %Y')
                            if isFinished and isSucced:
                                lastInfo = 'Last maintenance by ' + \
                                    str(row[0]) + ', date returned ' + \
                                    endDate + ', ' + ' (goods is able to use)'
            ...
            lastInfo = 'goods is new and able to use, date received ' + \
                dt.strftime('%d %B %Y')
        cur.close()
    ...
    def HasExists(self, idapp_fk_goods, serialnumber, datereq, daterel, fk_employee):
        ...
        filterdate = {'daterequest__range': [parse(startDateReq), parse(
            endateReq)], 'datereleased__range': [parse(startDateRel), parse(endateRel)]}
        ...
        try:
            with transaction.atomic():
                if Status == StatusForm.Input:
                    ...
                    param = {'FK_Goods': Data['idapp_fk_goods'], 'TypeApp': Data['typeapp'],
                             'SerialNumber': Data['serialnumber'], 'FK_Outwards': FKApp, 'CreatedBy': Data['createdby']}
                else:
    ...
    def getReportAdHoc(self, idapp):
        ...
        Query = """SELECT g.GoodsName,IFNULL(ngd.brandName,g.brandName) AS BrandName,ngo.SerialNumber,ngd.TypeApp AS `Type`,
				...
				IFNULL(emp2.employee_name,'') AS Eks_Employee,emp3.employee_name AS Sender
    ...
    def getreport_byrecipient(self, orderFields, sortIndice, pageSize, PageIndex, userName, columnKey, ValueKey, criteria=CriteriaSearch.Like, typeofData=DataType.VarChar):
        ...
        if columnKey == 'goods':
            ...
            colKey = 'nga.datereleased'
        elif columnKey == 'isGoodNew':
            colKey = 'nga.isnew'
            if ValueKey in ['YESS', 'yes', 'yess', 'Yess', '1', 'benar', 'betul', 'ya']:
                valKey = 1
            elif ValueKey in['NO', 'no', '0', 'tidak', 'bukan']:
                valKey = 0
        elif columnKey == 'for_employee':
            colKey = 'e.employee_name'
        elif columnKey == 'responsible_by':
            colKey = 'nga.responsible_by'
        elif columnKey == 'senderby':
            colKey = 'nga.senderby'
        elif columnKey == 'refgoodsfrom':
            colKey = 'ref.refgoodsfrom'
        ...
        Query = """ CREATE TEMPORARY TABLE T_RepByRecipient_Manager_""" + userName + """ ENGINE=MyISAM AS (
		        ...
		        CASE
			        WHEN(nga.fk_usedemployee IS NOT NUll) THEN(SELECT employee_name FROM employee WHERE idapp = nga.fk_usedemployee LIMIT 1)
			        END AS eks_employee, ref.refgoodsfrom,nga.createdby,nga.createddate,nga.equipment_desc,nga.descriptions
		        FROM n_a_goods_outwards nga INNER JOIN n_a_goods g ON g.IDApp = nga.FK_Goods
		        INNER JOIN n_a_goods_receive ngr ON ngr.FK_goods = nga.FK_Goods
		        INNER JOIN n_a_goods_receive_detail ngd ON ngd.FK_App = ngr.IDApp
		        AND nga.SerialNumber = ngd.SerialNumber
		        INNER JOIN (SELECT ng.IDApp,CASE
								        ...
								        END AS refgoodsfrom FROM n_a_goods_Outwards ng)ref ON Ref.IDApp = nga.IDApp
			    INNER JOIN employee e on e.IDApp = nga.FK_Employee
                INNER JOIN(SELECT ngo.FK_Employee,ngo.FK_goods,MAX(ngo.datereleased) dr FROM n_a_goods_outwards ngo GROUP BY ngo.FK_Employee,ngo.FK_Goods)FilL
                ON fill.FK_Employee = nga.FK_Employee AND fill.FK_goods = nga.FK_goods and fill.dr = nga.datereleased
        ...
        if orderFields != '':
            #Query = """SELECT * FROM T_Receive_Manager """ + (("ORDER BY " + ",".join(orderFields)) if len(orderFields) > 1 else " ORDER BY " + orderFields[0]) + (" DESC" if sortIndice == "" else sortIndice) + " LIMIT " + str(pageSize*(0 if PageIndex <= 1 else PageIndex)) + "," + str(pageSize)
            Query = """SELECT * FROM T_RepByRecipient_Manager_""" + userName + """ ORDER BY """ + orderFields + \
                (" DESC" if sortIndice == "" else ' ' + sortIndice) + \
                " LIMIT " + strLimit + "," + str(pageSize)
        else:
            Query = """SELECT * FROM T_RepByRecipient_Manager_""" + userName + \
                """ ORDER BY IDApp LIMIT """ + strLimit + "," + str(pageSize)
        cur.execute(Query)
        result = query.dictfetchall(cur)
        # get countRows
        Query = """SELECT COUNT(*) FROM T_RepByRecipient_Manager_""" + userName
        cur.execute(Query)
        row = cur.fetchone()
        totalRecords = row[0]
        cur.close()
