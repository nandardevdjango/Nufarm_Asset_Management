	...
	def Sql(self):
		if self.criteria==CriteriaSearch.Beetween:
			...
			if self.typeofData==DataType.Char or self.typeofData==DataType.VarChar or self.typeofData==DataType.NVarChar:
				...
				ResolveCriteria.__query = ' = {0}'.format(1 if self.valueData == 1 or self.valueData == '1' or self.valueData == True or self.valueData == 'true' or self.valueData == 'True' else 0)
...
class commonFunct:
	...
	def response_default(data):
		#"""this is default HttpResponse, use it to correct and neat response param
