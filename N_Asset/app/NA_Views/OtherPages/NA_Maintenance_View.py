...
def ShowCustomFilter(request):
    if request.is_ajax():
        ...
        cols.append({'name':'goodstype','value':'typeapp','selected':'','dataType':'varchar','text':'Goods type'})
        cols.append({'name':'serialnumber','value':'serialnumber','selected':'','dataType':'varchar','text':'Serial Number'})
        cols.append({'name':'requestdate','value':'requestdate','selected':'','dataType':'datetime','text':'Date Requested'})
        cols.append({'name':'startdate','value':'startdate','selected':'','dataType':'datetime','text':'Start Date'})
        cols.append({'name':'enddate','value':'enddate','selected':'','dataType':'datetime','text':'End Date'})
        cols.append({'name':'isstillguarante','value':'isstillguarante','selected':'','dataType':'boolean','text':'Still Has Warranty'})
        cols.append({'name':'maintenanceby','value':'maintenanceby','selected':'','dataType':'varchar','text':'Maintenance By'})
        cols.append({'name':'personalname','value':'personalname','selected':'','dataType':'varchar','text':'Person Name'})
        cols.append({'name':'isfinished','value':'isfinished','selected':'','dataType':'boolean','text':'Is Finished'})
        cols.append({'name':'issucced','value':'issucced','selected':'','dataType':'boolean','text':'Is Succed'})
        cols.append({'name':'expense','value':'expense','selected':'','dataType':'decimal','text':'Cost Expense'})
...
def NA_MaintenanceGetData(request):
    ...
    if(Isord is not None and str(Isord) != '') or (Isidx is not None and str(Isidx) != ''):
    	NAData = NAMaintenance.objects.PopulateQuery(str(Isidx),Isord,Ilimit, request.GET.get('page', '1'),request.user.username,IcolumnName,IvalueKey,criteria,dataType)#return tuples
    else:
    	NAData = NAMaintenance.objects.PopulateQuery('','DESC',Ilimit, request.GET.get('page', '1'),request.user.username,IcolumnName,IvalueKey,criteria,dataType)#return tuples
    ...
    for row in dataRows:
        ...
        datarow = {"id" :row['idapp'], "cell" :[row['idapp'],i,row['goods'],row['itemcode'],row['serialnumber'],row['requestdate'],\
            row['startdate'],row['isstillguarantee'],row['expense'],row['maintenanceby'],row['personalname'],row['enddate'], row['isfinished'],row['issucced'],\
...
def SearchGoodsbyForm(request):
    ...
    NAData = NAMaintenance.objects.search_M_ByForm(goodsFilter,str(Isidx),Isord,PageSize,PageIndex,request.user.username,commonFunct.str2bool(includeNotYetUsed))
    if NAData == []:
        results = {"page": "1","total": 0 ,"records": 0,"rows": [] }
    else:
